...
void printCommandList(CmdNode *head) {
  ...
  while (p != NULL) {
    ...
    if (p->loop_head != NULL) {
      CmdNode *loop_p = p->loop_head;
      while (loop_p != NULL) {
        if (strcmp(loop_p->cmd, "forward") == 0 ||
            strcmp(loop_p->cmd, "backward") == 0 ||
            strcmp(loop_p->cmd, "colour") == 0 ||
            strcmp(loop_p->cmd, "loop") == 0) {
          printf("%3d:   %s %d\n", line, loop_p->cmd, loop_p->val);
        } else {
          printf("%3d:   %s\n", line, loop_p->cmd);
        }
        line++;
...
int countCommands(CmdNode *head) {
  ...
  while (p != NULL) {
    count++;
    if (p->loop_head != NULL) {
      count += countCommands(p->loop_head);
    }
    p = p->next;
  }
  return count;
...
void insertCommandLoop(CmdNode *loop_node, CmdNode *new_CmdNode) {
  if (loop_node == NULL || new_CmdNode == NULL) {
    return;
  }
...
void executeCommand(CmdNode *cmd, int *x, int *y, int *direction, int *penDown, int *color, Image *im, int dx[], int dy[]) {
  if (strcmp(cmd->cmd, "penup") == 0) {
    *penDown = 0;
  }
  else if (strcmp(cmd->cmd, "pendown") == 0) {
    *penDown = 1;
  }
  else if (strcmp(cmd->cmd, "colour") == 0) {
    *color = cmd->val;
  }
...
void run(Image *im, CmdNode *head, int *endX, int *endY) {
  ...
  while (current != NULL) {
    if (strcmp(current->cmd, "loop") == 0) {
      int iterations = current->val;

      for (int i = 0; i < iterations; i++) {
        CmdNode *loop_current = current->loop_head;

        while (loop_current != NULL) {
          executeCommand(loop_current, &x, &y, &direction, &penDown, &color, im, dx, dy);
          loop_current = loop_current->next;
        }
      }
    } else {
      executeCommand(current, &x, &y, &direction, &penDown, &color, im, dx, dy);
    }
